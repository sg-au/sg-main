...
// Check if the folder "controllers" exists (in the same directory as this file)
...
if (fs.existsSync(controllersFolder)) {
  // Read all folders in the controllers folder
  const folders = fs.readdirSync(controllersFolder, { withFileTypes: true });
  folders.forEach((folder) => {
    if (!folder.isDirectory()) {
      return;
    }

    const controllerName = folder.name;

    router.use(
      `/${controllerName}/api`,
      require(`./controllers/${controllerName}/api.js`)
    );

    router.use(
      `/${controllerName}`,
      renderMiddleware(controllerName),
