...
const baggageScannerSubmissionsFile = "./data/baggage-scanner-survey-submissions.json";
...
router.get("/", async (req, res) => {
  ...
  var poolCount = (await axios.get(`${apiUrl}/pools`, axiosConfig)).data.meta
    .pagination.total;
  var servicesCount = (await axios.get(`${apiUrl}/services`, axiosConfig)).data
    .meta.pagination.total;
  var reviewsCount = (await axios.get(`${apiUrl}/reviews`, axiosConfig)).data
    .meta.pagination.total;
...
router.get("/announcements", (req, res) => {
  ...
  axios
    .get(
      `${process.env.STRAPI_API_URL}/announcements?populate=department.name&populate=department.profile.profile_pic&sort[0]=createdAt:desc`,
      axiosConfig
    )
    .then((response) => {
...
router.get("/course-reviews/:id", (req, res) => {
  axios
    .get(
      `${process.env.STRAPI_API_URL}/courses/${req.params.id}?populate[0]=faculties&populate[1]=reviews&populate[2]=course_reviews&populate[3]=reviews.author`,
      axiosConfig
    )
    .then((response) => {
      // Ratings
      ...
      courseReviews.forEach((review) => {
        transparentTotal += review.attributes.transparent
          ? review.attributes.transparent
          : 0;
        transparentCount +=
          review.attributes.transparent && review.attributes.transparent > 0
            ? 1
            : 0;

        relatabilityTotal += review.attributes.relatability
          ? review.attributes.relatability
          : 0;
        relatabilityCount +=
          review.attributes.relatability && review.attributes.relatability > 0
            ? 1
            : 0;

        strictTotal += review.attributes.strict ? review.attributes.strict : 0;
        strictCount +=
          review.attributes.strict && review.attributes.strict > 0 ? 1 : 0;

        fairnessTotal += review.attributes.fair ? review.attributes.fair : 0;
        fairnessCount +=
          review.attributes.fair && review.attributes.fair > 0 ? 1 : 0;

        lecturerTotal += review.attributes.lecturer
          ? review.attributes.lecturer
          : 0;
        lecturerCount +=
          review.attributes.lecturer && review.attributes.lecturer > 0 ? 1 : 0;

        overallTotal += review.attributes.overall
          ? review.attributes.overall
          : 0;
        overallCount +=
          review.attributes.overall && review.attributes.overall > 0 ? 1 : 0;
      });
      ...
      const transparent =
        transparentCount > 0
          ? Math.round((transparentTotal / transparentCount) * 10) / 10
          : 0;
      const relatability =
        relatabilityCount > 0
          ? Math.round((relatabilityTotal / relatabilityCount) * 10) / 10
          : 0;
      const strict =
        strictCount > 0 ? Math.round((strictTotal / strictCount) * 10) / 10 : 0;
      const fair =
        fairnessCount > 0
          ? Math.round((fairnessTotal / fairnessCount) * 10) / 10
          : 0;
      const lecturer =
        lecturerCount > 0
          ? Math.round((lecturerTotal / lecturerCount) * 10) / 10
          : 0;
      const overall =
        overallCount > 0
      ...
      const rating =
        Math.round(
          ((transparent + relatability + strict + fair + lecturer + overall) /
            6) *
            10
        ) / 10;
      ...
      response.data.data.attributes.description =
        response.data.data.attributes.description.replace(/<h4>.*?<\/h4>/, "");
      response.data.data.attributes.description =
        response.data.data.attributes.description.replace(
          '<p class="cmsDescp">',
          ""
        );
      response.data.data.attributes.description =
        response.data.data.attributes.description.replace("</p>", "");
      // Description
      ...
      res.render("platform/pages/course", {
...
router.get("/add-course-review/:id", (req, res) => {
  axios
    .get(
      `${process.env.STRAPI_API_URL}/courses/${req.params.id}?populate[0]=faculties&populate[1]=course_reviews`,
      axiosConfig
    )
    .then((response) => {
      response.data.data.attributes.description =
      ...
      response.data.data.attributes.description =
        response.data.data.attributes.description.replace("</p>", "");
      res.render("platform/pages/add-course-review", {
        data: response.data.data.attributes,
        id: response.data.data.id,
      });
    })
  ...
  if (req.body.name && req.body.name === "on") {
    ...
    axios
      ...
      .then((response) => {
        ...
        axios
          .post(
            `${process.env.STRAPI_API_URL}/reviews`,
            { data: req.body },
            axiosConfig
          )
          .then((response) => {
    ...
    try {
      const response = await axios.get(
        process.env.STRAPI_API_URL +
          "/users?populate[tickets][populate][departments][fields][0]=name&filters[email][$eqi]=" +
...
router.get("/create-ticket", (req, res) => {
  axios
    .get(
      `${process.env.STRAPI_API_URL}/departments?populate[0]=profile.email`,
      axiosConfig
    )
    .then((response) => {
      // Access the response data
      ...
      const categoryList = publicTicketCategories.categories.map(
        (category) => category.name
      );

      // Extract SubcategoryList
      const subcategoryList = publicTicketCategories.categories.reduce(
        (acc, category) => {
      ...
      publicTicketCategories.categories.forEach((category) => {
        categorySubcategoryMap[category.name] = category.subcategories.map(
      ...
      res.render("platform/pages/create-ticket", {
        ...
        categorySubcategoryMap: JSON.stringify(categorySubcategoryMap),
        subcategoryMinistryMap: JSON.stringify(subcategoryMinistryMap),
      });
    })
    .catch((error) => {
      console.error("Error fetching departments:", error);
...
router.post("/create-ticket", (req, res) => {
  ...
  axios
    ...
    .then((response) => {
      ...
      axios
        .post(
          `${process.env.STRAPI_API_URL}/tickets`,
          {
            data: newTicketData,
          },
          axiosConfig
...
router.get("/tickets/:id", (req, res) => {
  const fetchData = async () => {
    try {
      const response = await axios.get(
        process.env.STRAPI_API_URL +
          "/users?populate[tickets][populate][departments][fields][0]=name&populate[tickets][populate][response][fields][0]=response&filters[email][$eqi]=" +
          req.user._json.email,
        axiosConfig
      );
      return response.data;
...
router.get("/public-forum/:id", async (req, res) => {
  try {
    ...
    var response = await axios.get(
      `${apiUrl}${endpoint}/${petitionID}?populate=signatures,comments,department&pagination[pageSize]=1000`,
      axiosConfig
    );
    ...
    res.render("platform/pages/petition", {
      ...
      comments: com_array,
      signed: user_array.includes(req.user._json.email),
    });
  } catch (error) {
    console.error("An error occurred:", error);
  }
...
router.post("/create-comment", async (req, res) => {
  ...
  var user_response = await axios.get(
  ...
  try {
    ...
    var com_response = await axios.post(
      `${apiUrl}${com_endpoint}`,
      commentData,
      axiosConfig
    );
...
router.post("/sign-petition", async (req, res) => {
  ...
  var petitionId = Number(req.body.petitionId);
  var response = await axios.get(
    `${apiUrl}${endpoint}/${petitionId}?populate=signatures,comments`,
    axiosConfig
  );
  var user = (
    await axios.get(
  ...
  try {
    var signPetition = await axios.put(
      `${apiUrl}${endpoint}/${petitionId}`,
      petitionUpdate,
      axiosConfig
    );
...
router.post("/feedback", (req, res) => {
  ...
  const mailOptions = {
    ...
    html: `
      ...
      <p class="greeting">Dear member,<hr/><br> This mail is to inform you of a receipt of feedback from ${
        req.user._json.name
      }. Please provide necessary support to the member or mark the bug to be solved. <br/><br/>Thank you for the cooperation.</p>
      <table>
          <tr>
              <th>Field</th>
              <th>Value</th>
...
router.get("/sg-compose-outbox", async (req, res) => {
  ...
  axios
    .get(
      `${process.env.STRAPI_API_URL}/users?filters[email][$eqi]=${userEmail}&populate=sg_mails`,
      axiosConfig
    )
    .then((response) => {
      // Assuming you get a single user with the specified email
      user = response.data[0];
      res.render("platform/pages/sg-compose-outbox", {
        phone: user.phone,
        mails: user.sg_mails,
      });
    })
...
router.get("/sg-compose/dashboard", async (req, res) => {
  try {
    ...
    if (process.env.HOR_MEMBERS_LIST.includes(req.user._json.email)) {
      ...
      var response = await axios.get(
        `${apiUrl}${endpoint}?populate=sender&pagination[pageSize]=${3000}`,
        axiosConfig
      );
  ...
  try {
    ...
    if (!aliasvalid) {
      ...
      const response = await axios.post(
        `${process.env.STRAPI_API_URL}/sg-mails`,
        { data: req.body },
        axiosConfig
      );
      ...
      await new Promise((resolve, reject) => {
        transporterSG.sendMail(mailOptions, async (error, info) => {
          if (error) {
            ...
            await axios.put(
              `${apiUrl}/sg-mails/${req.body.mailid}`,
              { data: req.body },
              axiosConfig
            );
            console.log("Email sent successfully!", info.messageId);
...
router.get("/pool-cab", async (req, res) => {
  try {
    ...
    var user_filled = await axios.get(
      `${apiUrl}/users?filters[email][$eqi]=${req.user._json.email}&fields[0]=phone&populate=pools`,
      axiosConfig
    );
    ...
    for (var i = 0; i < user_detail.length; i++) {
      ...
      if (
        user_detail[i].status == "available" &&
        acceptableDates.includes(
          `${
            poolDate.getMonth() + 1
...
router.post("/pool-submit", async (req, res) => {
  ...
  axios
    .post(
      `${process.env.STRAPI_API_URL}/pools`,
      { data: req.body },
      axiosConfig
    )
    .then((response) => {
...
router.post("/update-pool", async (req, res) => {
  var pool = await axios.get(
    `${apiUrl}/pools/${atob(req.body.pool_id)}`,
    axiosConfig
  );
...
router.get("/cancel-cab-pool", async (req, res) => {
  ...
  var userpools = await axios.get(
    `${apiUrl}/users?filters[email][$eqi]=${req.user._json.email}&populate=pools`,
    axiosConfig
  );
...
router.get("/cgpa-planner", async (req, res) => {
  ...
  if (cgpa_data == null) {
    ...
    res.render("platform/pages/cgpa-planner2", {
      cgpa_data: cgpa_data,
...
router.get("/pool-service", async (req, res) => {
  try {
    var user_filled = await axios.get(
      `${apiUrl}/users?filters[email][$eqi]=${req.user._json.email}&fields[0]=phone&populate=services`,
      axiosConfig
    );
    ...
    if (user_services.length == 0 || userActiveServices == 0) {
      res.render("platform/pages/pool-service-form", {
        phone: phone,
        currentDate: currentDate,
      });
    } else if (user_services.length == 1 || userActiveServices == 1) {
      var services_data = await axios.get(
        `${apiUrl}/services?populate=user&pagination[pageSize]=2000&_where[end_lte]=${new Date()}`,
        axiosConfig
      );
      services_data = services_data.data.data;
      res.render("platform/pages/pool-service", {
        services: services_data,
        user_services: user_services[0],
      });
    } else {
      var services_data = await axios.get(
        `${apiUrl}/services?populate=user&pagination[pageSize]=2000&_where[end_lte]=${new Date()}`,
      ...
      res.render("platform/pages/pool-service", {
        services: services_data,
        user_services: user_services[i - 1],
      });
    }
  } catch (error) {
    console.error("An error occurred:", error);
...
router.get("/cancel-pool-service", async (req, res) => {
  ...
  // var pool = (await axios.get(`${apiUrl}/pools?[pooler][email]=${userEmail}&filters[status][$eqi]=available`, axiosConfig));
  // console.log(pool.data.data);
  var userservices = await axios.get(
    `${apiUrl}/users?filters[email][$eqi]=${req.user._json.email}&populate=services`,
    axiosConfig
  );
  // console.log(userpools.data[0].pools);
  obj = [];
...
router.post("/pool-service", async (req, res) => {
  ...
  axios
    .post(
      `${process.env.STRAPI_API_URL}/services`,
      { data: req.body },
      axiosConfig
    )
    .then((response) => {
...
router.get("/assets", async (req, res) => {
  var user_data = await axios.get(
    `${apiUrl}/users?filters[email][$eqi]=${req.user._json.email}`,
    axiosConfig
  );
  var user = user_data.data[0];
  var assets = await axios.get(
    `${apiUrl}/assets?populate=last_borrow_request`,
    axiosConfig
  );
  assets = assets.data.data;
  res.render("platform/pages/assets", {
    assets: assets,
    user: user,
...
router.get("/assets/dashboard", async (req, res) => {
  if (process.env.BORROW_POC.includes(req.user._json.email)) {
    ...
    res.render("platform/pages/assets-dashboard", {
      requests: borrow_data.data.data,
    });
  } else {
    res.send("error 404");
  }
...
router.get("/assets/accept/:id", async (req, res) => {
  if (process.env.BORROW_POC.includes(req.user._json.email)) {
    var borrow_data = await axios.get(
      `${apiUrl}/borrow-requests/${req.params.id}`,
      axiosConfig
    );
...
router.post("/assets", async (req, res) => {
  ...
  var assetData = await axios.get(
    `${apiUrl}/assets/${req.body.asset}?populate=last_borrow_request`,
    axiosConfig
  );
  if (
    assetData.data.data.attributes.last_borrow_request.data == null ||
    (assetData &&
      hasDatePassed(
        assetData.data.data.attributes.last_borrow_request.data.attributes.to
      ))
  ) {
    axios
      .post(
        `${process.env.STRAPI_API_URL}/borrow-requests`,
        { data: req.body },
        axiosConfig
      )
      .then((response) => {
        ...
        const mailOptions = {
          ...
          subject:
            ...
            assetData.data.data.attributes.name +
            " from " +
            helpers.borrowDate(new Date(req.body.from)) +
            " to " +
            helpers.borrowDate(new Date(req.body.to)),
          html: `
                <!DOCTYPE html>
                  <html lang="en">
                  <head>
                  ...
                  <body>
                      <div class="container">
                          ...
                          <div class="undertaking">
                              ...
                              <p>I, <strong>${
                                req.user._json.name
                              }</strong>, a student of Ashoka University, hereby agree to the following terms and conditions for borrowing devices from the Ministry of Technology:</p>
                              ...
                              <ul>
                                  <li>I am borrowing the device from <strong>${helpers.borrowDate(
                                    new Date(req.body.from)
                                  )}</strong> to <strong>${helpers.borrowDate(
            new Date(req.body.to)
          )}</strong>, not exceeding the maximum borrowing period of two weeks.</li>
                              ...
                              <p><strong>Device Description:</strong> ${
                                req.body.deviceDescription
                              }</p>
    ...
    await axios.put(
      `${apiUrl}/assets/${req.body.asset}`,
      { data: { booked_until: req.body.to } },
      axiosConfig
    );
  } else {
...
router.get("/baggage-scanner-survey", async (req, res) => {
  try {
    ...
    let submissions = [];
    try {
      submissions = JSON.parse(fs.readFileSync(baggageScannerSubmissionsFile, "utf8"));
    } catch (err) {
      // File doesn't exist yet, create empty array
      submissions = [];
    }
    ...
    if (submissions.includes(hashedEmail)) {
      // User has already submitted
      res.render("platform/pages/baggage-scanner-survey-submitted");
    } else {
      // Show the survey form
      res.render("platform/pages/baggage-scanner-survey");
    }
  } catch (error) {
    console.error("Error in baggage scanner survey GET:", error);
    res.status(500).send("An error occurred");
  }
});

router.post("/baggage-scanner-survey", async (req, res) => {
  try {
    // Hash the user's email
    const hashedEmail = helpers.hash(req.user._json.email);
    ...
    // Add row to Google Sheet using the existing sheets client
    await sheets.spreadsheets.values.append({
      spreadsheetId: process.env.BAGGAGE_SCANNER_SURVEY_SHEET_ID,
      range: 'Sheet1!A:H', // Updated range to include question6
      valueInputOption: 'USER_ENTERED',
      resource: {
        values: values
    ...
    // Add hashed email to submissions file
    submissions.push(hashedEmail);
    fs.writeFileSync(baggageScannerSubmissionsFile, JSON.stringify(submissions, null, 2));
    
    res.json({ success: true, message: "Survey submitted successfully" });
    
  } catch (error) {
    console.error("Error submitting baggage scanner survey:", error);
    res.status(500).json({ error: "An error occurred while submitting the survey" });
  }
});
